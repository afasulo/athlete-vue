...
class DataProcessor:
    ...
    def get_player_stats(user_id, date_from=None, date_to=None, min_pitch_speed=None, max_pitch_speed=None):
        ...
        stats = {
            ...
            'avg_exit_velo': np.mean([p.exitvelo for p in plays if p.exitvelo and p.exitvelo > 0]) if plays else 0,
            'max_exit_velo': max([p.exitvelo for p in plays if p.exitvelo and p.exitvelo > 0], default=0),
            'avg_distance': np.mean([p.distance for p in plays if p.distance and p.distance > 0]) if plays else 0,
            'max_distance': max([p.distance for p in plays if p.distance and p.distance > 0], default=0),
    ...
    def _calculate_zone_stats(plays):
        ...
        zone_stats = {i: {'count': 0, 'avg_exit_velo': 0} for i in range(0, 15)}  # Include zone 0
        
        for play in plays:
            if play.quadrant is not None and play.exitvelo and play.exitvelo > 0:
                zone = zone_stats.get(play.quadrant, {'count': 0, 'avg_exit_velo': 0})
                zone['count'] += 1
                zone['avg_exit_velo'] = (
                    (zone['avg_exit_velo'] * (zone['count'] - 1) + play.exitvelo) 
                    / zone['count']
                )
    ...
    def _calculate_exit_velo_trends(plays):
        ...
        df = pd.DataFrame([{
            ...
            'exit_velo': p.exitvelo
        } for p in plays if p.exitvelo and p.exitvelo > 0])
